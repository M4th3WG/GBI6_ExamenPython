...
def download_pubmed(keyword):
    ...
    Datosbase = Entrez.read(Entrez.esearch(db="pubmed", 
                            term=keyword,
                            usehistory="y"))
    webenv = Datosbase["WebEnv"]
    query_key = Datosbase["QueryKey"]
...
def mining_pubs(tipo,archivo):
    a = re.sub(r'\..*\d.*\,',',',archivo)
    b = re.sub(r'\s+[Eceinlort]{10}\s+[aders]{7}.*','',a)
    c = re.sub(r'\..*\d.*','',b)
    d = re.sub(r'\s[\w._%+-]+@[\w.-]+\.[a-zA-Z]{1,4}','',c)
    x=d[1:].split('PMID-')
    ID=[]
    AU=[]
    DP=[]
    ...
    for obj in AP:
        ...
        AC[b]=g
        b=b+1
#al haber tantos paisesy con diferentes "teminaciones" o "presentaciones" lo que se busca es unificar los mayor posible para evitar errores en la ejecucición
    q=0
    AD=[0]*len(AP)
    for g in AC:
        if g == "USA":
    ...
    for elem in ADh:
        ...
        year = pd.DataFrame({'PMID' : ID,
                           'DP_year' : DP})

    auth = pd.DataFrame({'PMID' : ID,
                           'num_auth' : AU})

    auth1 = pd.DataFrame({'Country' : CO,
                           'num_auth' : AD})
